...
class Trader:
    ...
    def run(self, state: TradingState):      
        ...
        if (
            ...
            ### BASKET 2 ARBITRAGE ###
            if product_strings[Product.BASKET2] in state.order_depths:
                synthetic_basket2_price = 4 * mid_prices[Product.CROISSANTS] + 2 * mid_prices[Product.JAMS]
                spread = mid_prices[Product.BASKET2] - synthetic_basket2_price
                logger.print(f"Basket2 Price: {mid_prices[Product.BASKET2]}, Synthetic Basket2 Price: {synthetic_basket2_price}, Spread: {spread}")
                
                threshold = 49 # 30 is the mean of the spread (on historical data)
                
                # +spread -> basket2 is overpriced
                if spread > threshold:                    
                    # sell basket2 at bid 
                    self.safe_order(Product.BASKET2, best_bids[Product.BASKET2], -1, positions[Product.BASKET2], result[product_strings[Product.BASKET2]])
                    
                    # buy synthetic basket at ask
                    self.safe_order(Product.CROISSANTS, best_asks[Product.CROISSANTS], 4, positions[Product.CROISSANTS], result[product_strings[Product.CROISSANTS]])
                    self.safe_order(Product.JAMS, best_asks[Product.JAMS], 2, positions[Product.JAMS], result[product_strings[Product.JAMS]])
                
                # -spread -> basket2 is underpriced
                elif spread < -threshold:                    
                    # buy basket2 at ask
                    self.safe_order(Product.BASKET2, best_asks[Product.BASKET2], 1, positions[Product.BASKET2], result[product_strings[Product.BASKET2]])
                    
                    # sell components at bid
                    self.safe_order(Product.CROISSANTS, best_bids[Product.CROISSANTS], -4, positions[Product.CROISSANTS], result[product_strings[Product.CROISSANTS]])
                    self.safe_order(Product.JAMS, best_bids[Product.JAMS], -2, positions[Product.JAMS], result[product_strings[Product.JAMS]])
