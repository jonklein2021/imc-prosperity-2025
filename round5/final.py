...
class Trader:
    ...
    def calculate_popular_price(self, buy_orders, sell_orders):
        ...
        # get most common buy and sell prices
        bid_price = max(buy_orders, key=lambda t: t[1])[0]
        ask_price = min(sell_orders, key=lambda t: t[1])[0]
    ...
    def run(self, state: TradingState):
        ...
        if product_strings[Product.KELP] in state.order_depths:
            # calculate fair price
            fair_price = self.calculate_popular_price(state.order_depths[product_strings[Product.KELP]].buy_orders, state.order_depths[product_strings[Product.KELP]].sell_orders)
            
            # take best orders
            self.take_best_orders(Product.KELP, result[product_strings[Product.KELP]], state.order_depths[product_strings[Product.KELP]], fair_price, 1, positions[Product.KELP])
